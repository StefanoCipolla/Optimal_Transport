...
% -------------------------------------------------------------------------------------------------------------------- %
   ...
   switch pc
        ...
        if (iter > 1)
            ...
            %sigma  = 1e3* min(min(x),min(z)).^2/mu;
            sigma  = 1e3* min(min(x(pos_vars)),min(z(pos_vars))).^2/mu;
        else
            sigma = 0.5;
        end
        ...
        % ------------------------------------------------------------------------------------------------------------ %
        [dx,dy,dz,instability] = Newton_backsolve(NS,res_p,res_d,res_mu,pos_vars,free_variables,Struct,x,z,mu);
        if (instability == true) % Checking if the matrix is too ill-conditioned. Mitigate it.
            if (retry < max_tries)
                ...
                iter = iter -1;
                retry = retry + 1;
                reg_limit = max(reg_limit*100,tol);
                continue;
            else
    ...
    % ---------------------------------------------------------------------------------------------------------------- %
        ...
        if (instability == true) % Checking if the matrix is too ill-conditioned. Mitigate it.
            if (retry_p < max_tries)
                ...
                iter = iter -1;
                retry_p = retry_p + 1;
                reg_limit = min(0.5,max(reg_limit*100,tol));
                continue;
            else
                fprintf('The system matrix is too ill-conditioned.\n');
                OInfo.opt = 2;
    ...
    % ---------------------------------------------------------------------------------------------------------------- %
        ...
        %                                                               solve the original problem in 1 iteration.
        % ------------------------------------------------------------------------------------------------------------ %
        [dx_c,dy_c,dz_c,instability] = Newton_backsolve(NS,zeros(m,1),zeros(n,1),res_mu,pos_vars,free_variables,Struct,x,z,mu);
        if (instability == true) % Checking if the matrix is too ill-conditioned. Mitigate it.
    ...
    % ================================================================================================================ %
        ...
        for i = 1: K_c
            ...
            if (alpha_xn < 1.01*(alpha_x) &&  alpha_zn < 1.01*(alpha_z)) || (alpha_xn < 1e-1*(alpha_x))  ||  (alpha_zn < 1e-1*(alpha_z))
                %alpha_x = alpha_xn;
                %alpha_z = alpha_zn;
              break;
            else
